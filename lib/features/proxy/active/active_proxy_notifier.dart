...
import 'package:uflow/core/utils/throttler.dart';
import 'package:uflow/features/connection/notifier/connection_notifier.dart';
import 'package:uflow/features/proxy/data/proxy_data_providers.dart';
...
class IpInfoNotifier extends _$IpInfoNotifier with AppLogger {
  ...
  Future<oldipinfo.IpInfo> build() async {
    ...
    final info = await ref.watch(proxyRepositoryProvider).getCurrentIpInfo(cancelToken).getOrElse((err) {
      ...
      // throw err; //uflow: remove exception to be logged
      throw const UnknownIp();
    }).run();
